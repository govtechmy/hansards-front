...
export default async function handler(
  ...
  const { GET_COUNTS, GET_TOKEN, POST_DL, POST_SHARE, POST_TOKEN, TINYBIRD_API } = process.env
  ...
  if (req.method === "POST") {
    ...
    const name = event === "download" ? POST_DL : event === "share" ? POST_SHARE : ""
    return await post(`/events?name=${name}&wait=true`,
      req.body,
      "https://api.tinybird.co/v0",
      {
    ...
    return await get(
      ...
      .then(({ data }: { data: { data: { type: string, counts: number }[] } }) =>
        res.status(200).json(
          data.data.reduce((acc, event) => {
